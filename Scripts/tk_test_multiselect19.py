...
window.geometry('600x800')                 # Menu size
...
frmBottom.grid_columnconfigure(0, weight=6)
...
# Row 0
entFilter = tk.Entry(master=frmEntries, width=30, font=("Helvetica", 12))
...
lblFilter.grid(row=0, column=1, rowspan=2, columnspan=3, sticky="W")
...
entFolder = tk.Entry(master=frmEntries, width = 40, font=("Helvetica", 12))
...
entDBinput = tk.Entry(master=frmEntries, width = 40, font=("Helvetica", 12))
...
entDBoutput = tk.Entry(master=frmEntries, width = 40, font=("Helvetica", 12))
...
#lblCRS = tk.Label(master=frmEntries, text="Output coordinate system")
#lblCRS.grid(row=8, column=0, columnspan=3, sticky="W")

# Listbox for outputcoordinate system
crsList = tk.StringVar()
#lbxCRS = tk.Listbox(master=frmBottom, listvariable=crsList, width=30, height=20, selectmode='single')
#lbxCRS.grid(row=9, column=0, columnspan=2, sticky="W")
...
cbxOverwrite.grid(row=10, column=0, sticky="W")
...
lbxDownloads.grid(row=1, column=0, rowspan=4, columnspan=3, sticky="W")
lbxDownloads.bind("<ButtonRelease-1>", ShowSelected)

# Label for selection count
lblCount = tk.Label(master=frmBottom, text="", anchor="w", width=40, background=gray)
lblCount.grid(row=1, column=3)
lblCount.config(text="No selection yet")
