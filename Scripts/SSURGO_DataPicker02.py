...
def errorMsg(excInfo):
    ...
    try:
        ...
        if len(excInfo) == 3:
            ...
            exc_type, exc_value= excInfo
            PrintMsg(".\tTwo values for sys.exc_info()... exc_type: " + str(exc_type) + "\texc_value: " + str(exc_value), 0)

        else:
            PrintMsg(".\tLength of sys.exc_info(): " + str(len(excInfo)), 0)

        PrintMsg("** " + 70 * "=", 2)
        sys.exit("Stopping program due a raised error")
...
import sys, string, os, traceback, locale, time, datetime, shutil
...
##    bSpatialDiet = arcpy.GetParameter(7)          # Compress polygon geometries on import (spatialite only at this time)
##    bAppend = arcpy.GetParameter(8)               # Use arcpy.Append command to import spatial data. Default = False.
##    bTabularOnly = arcpy.GetParameter(9)          # Skip spatial import (shapefiles). This database will contain only tabular data

    # Non-arcpy method for handling script arguments
    #
    inputFolder = sys.argv[1]                     # location of SSURGO datasets containing spatial folders
    ssaLayer = sys.argv[2]                        # Test to see if I can sort the ssaLayer when using the 'Create SSURGO-Lite DB by Map' tool
    templateName = sys.argv[3]                    # empty SQLite database with SSSURGO schema
    newDB = sys.argv[4]                           # fullpath for the new output geodatabase
    geogRegion = sys.argv[5]                      # Geographic Region used to set output coordinate system
    surveyList = sys.argv[6]                      # list of SSURGO dataset folder names to be proccessed (e.g. 'soil_ne109')
    tmpFolder = sys.argv[7]                       # Folder where vacuum will take place
    bSpatialDiet = sys.argv[8]                    # Compress polygon geometries on import (spatialite only at this time)
    bAppend = sys.argv[9]                         # Use arcpy.Append command to import spatial data. Default = False.
    bTabularOnly = sys.argv[1]                    # Skip spatial import (shapefiles). This database will contain only tabular data

    bVerbose = False

    # Test MyError class
    # raise MyError("TESTING MyError")

    # convert surveyList from delimited string to a python list
    surveyList = surveyList.split(";")

    # convert boolean strings to python boolean values
    if bSpatialDiet == 'true':
        bSpatialDiet = True
    ...
    args = [inputFolder, ssaLayer, templateName, newDB, geogRegion, surveyList, tmpFolder, bSpatialDiet, bAppend, bTabularOnly]

    if bVerbose:
        PrintMsg("Listing parameter values for " + os.path.basename(__file__), 0)
    ...
    for arg in args:
        if arg is None or arg == '':
            ...
            if bVerbose:
               PrintMsg(".\tparameter: " + str(arg), 0)

            else:
                pass

    if bVerbose:
        # Test to see how arcpy.GetParameter compares to sys.argv[]
        testArg1= sys.argv[6]
        PrintMsg(".", 0)
        PrintMsg(".\tsys.argv surveyList: " + str(testArg1), 0)
        testArg2 = sys.argv[10]
        PrintMsg(".\tsys.argv TabularOnly: " + str(testArg2), 0)

    import SSURGO_DataLoader02  # For production version of tool, change this back to 'import'
    ...
    if bDB:
        bGood = SSURGO_DataLoader02.main(inputFolder, surveyList, newDB, geogRegion, tmpFolder, bSpatialDiet, bAppend, bTabularOnly)
