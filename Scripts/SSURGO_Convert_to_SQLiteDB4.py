# SSURGO_Convert_to_SQLiteDB4.py
...
#
# ArcGIS Pro 2.7 which uses Python library: sqlite3 version 2.6
...
##        logFile = os.path.join(tmpFolder, "log_gp.txt")
##
...
def CreateSSURGO_DB(newDB, areasymbolList, aliasName, templateDB):
    ...
    try:
        ...
        if dbExt == ".geodatabase":
            ...
            PrintMsg(".\tUsing arcpy to copy template to new folder: " + os.path.dirname(newDB), 0)
...
def ImportTabularSQL1(newDB, pathList, dbVersion, tableList, conn, liteCur):
    ...
    try:
        ...
        #PrintMsg("Current workspace: " + env.workspace, 0)
        PrintMsg(".", 0)
        PrintMsg(" \nImporting tabular data using function 'ImportTabularSQL1'...", 0)
...
def ImportTabularSQL2(newDB, pathList, dbVersion, tableList, conn, liteCur):
    ...
    try:
        ...
        #PrintMsg("Current workspace: " + env.workspace, 0)
        PrintMsg(".", 0)
        PrintMsg(" \nImporting tabular data using function 'ImportTabularSQL2'...", 0)
        ...
        # Create a dictionary. Keys are tabular-textfile names with table information
##        PrintMsg(".\tGetting table names and iefilenames...", 0)
##        PrintMsg(".", 0)
##        PrintMsg(".", 0)
##        PrintMsg(".", 0)
##        PrintMsg(".", 0)
...
##        PrintMsg(".\tGot table names and iefilenames...", 0)
##        PrintMsg(".", 0)
##        PrintMsg(".", 0)
##        PrintMsg(".", 0)
##        PrintMsg(".", 0)
...
##        PrintMsg(".\tBeginning iteration by txtFiles...", 0)
##        PrintMsg(".", 0)
##        PrintMsg(".", 0)
##        PrintMsg(".", 0)
##        PrintMsg(".", 0)
...
##            PrintMsg(".\tNow at the top of the table iteration and getting tablename from tblInfo dictionary...", 0)
##            PrintMsg(".", 0)
##            PrintMsg(".", 0)
##            PrintMsg(".", 0)
##            PrintMsg(".", 0)
...
##            PrintMsg(".\tGetting fieldNames from fldInfos...", 0)
...
##            PrintMsg(".\tSet src variable. No data will actually be imported. Skipping the INSERTs...", 0)
##            PrintMsg(".", 0)
##            PrintMsg(".", 0)
##            PrintMsg(".", 0)
##            PrintMsg(".", 0)
...
##                                # This method of checking input cinterp.txt won't work if a survey is NOTCOM
##                                PrintMsg(".\tChecking item count in first row...", 0)
##                                PrintMsg(".", 0)
##                                PrintMsg(".", 0)
##                                PrintMsg(".", 0)
##                                time.sleep(5)
##                                itemCnt = len(rows[0])
##                                PrintMsg(".\tGot item count of " + str(itemCnt) + " values in first row", 0)
##                                PrintMsg(".", 0)
##                                PrintMsg(".", 0)
##                                PrintMsg(".", 0)
##                                time.sleep(5)
...
##                                if itemCnt != 19:
##
##                                    PrintMsg(".\tImporting original SSURGO data...", 0)
##                                    PrintMsg(".", 0)
##                                    PrintMsg(".", 0)
##                                    PrintMsg(".", 0)
##                                    time.sleep(5)
...
##                                else:
                                # This is a standard SSURGO export that contains all of the legancy cointerp columns
                                #
##                                PrintMsg(".\tImporting cointerp data...", 0)
##                                PrintMsg(".", 0)
##                                PrintMsg(".", 0)
##                                PrintMsg(".", 0)
##                                time.sleep(5)
...
## ===================================================================================
##def GetInputFieldInfo(dsShape, inLayer, inLayerDefn):
##    # Get input field information from first shapefile.
##    # For now we are assuming that the rest are the same
##
##    try:
##        PrintMsg(".\tGetting input layer definition from first shapefile", 0)
##
##        #PrintMsg(".\tGetting field count for input shapefile", 0)
##        #time.sleep(5)
##
##        inFldCount = inLayerDefn.GetFieldCount()
##        PrintMsg(".", 0)
##        PrintMsg(".\tGot field count for input shapefile: " + str(inFldCount), 0)
##        #PrintMsg(".\t", 0)
##        #PrintMsg(".\t", 0)
##
##
...
##                #time.sleep(10)
##                inFieldDefs.append(inLayerDefn.GetFieldDefn(i))
##
##            except:
...
def AppendFeatures(newDB, geogRegion, mupolyList, mulineList, mupointList, sflineList, sfpointList, sapolyList, featCnt):
    ...
    #
    # BIG PROBLEM? Using ogr to handle geometry seems to be running into the Valid geometry problem where ArcPy doesn't.
    # I need to first add my features-in-features-out checks.
    ...
    try:
        ...
        if gpkgDriver is None:
            err = "Failed to get OGR inputDriver for " + newDB
            raise MyError(err)

        gdal.SetConfigOption('OGR_GPKG_FOREIGN_KEY_CHECK', 'NO')

        dsGPKG = gpkgDriver.Open(newDB, 1)

        if dsGPKG is None:
            err = "Failed to open " + newDB + " using ogr with " + gpkgDriver.name + " driver"
            raise MyError(err)
        ...
        if len(mupolyList) > 0:
            ...
            outLayer = dsGPKG.GetLayerByName(outLayerName)
        ...
        if len(mulineList) > 0:
            ...
            PrintMsg(".", 0)
            PrintMsg("Skipping mapunit lines...", 0)
        ...
        if len(dBadGeometry) > 0:
            ...
            for shpFile, shpIDs in dBadGeometry.items():
                PrintMsg(".\tShapefile " + shpFile + " has " + str(len(shpIDs)) + " polygons with invalid geometry: " + str(shpIDs), 0)
...
## ===================================================================================
def main(inputFolder, surveyList, newDB, geogRegion, bClipped, tileInfo, templateDB, areasymbolList):
...
##        # END OF RUNNING MDSTAT
        ...
        if len(mupolyList) > 0:
            ...
            if bGeodatabase:
                ...
                # ogr driver selection. May need to move this. Not sure if it is being used.
                if dbExt == ".gdb":
                ...
                ##    bMD = ImportMDTabularSQL2(newDB, dbPath, tableList, conn, liteCur)  # new, import md tables from text files of last survey area
                ##
                ## elif dbExt == ".geodatabase":
                ##    bMD = ImportMDTabularGDB(newDB, dbPath, tableList)  # new, import md tables from text files of last survey area
                ##
                ## if bMD == False:
                ##    raise MyError("")
...
try:
    if __name__ == "__main__":
        ...
        bGood = main(inputFolder, surveyList, newDB, geogRegion, tileInfo, False, templateDB, areasymbolList)
