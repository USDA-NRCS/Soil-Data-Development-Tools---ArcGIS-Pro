    ...
    try:
        ...
        if not excInfo is None:
            ...
            if len(excInfo) > 0:
                ...
                PrintMsg("Error Type: " + errType + "; Value: " + str(errValue) + "; " + str(errTB), 1)
                ...
                if not errTB is None:
                    ...
                    try:
                        ...
                        PrintMsg("Problem getting info from traceback object", 1)
                    ...
                    #PrintMsg("ErrorMsg ended up with: " + msgTB, 0)
...
def GetDownload(areasym, surveyDate, importDB, newFolder):
    ...
    try:
        ...
        zipDL = request.urlopen(zipURL, data=None)
        time.sleep(1)
        ...
        zipCode = zipDL.code
        zipMD = zipDL.info()
        zipType = zipMD['content-type']
        zipSize = int(zipMD['content-length'])
        ...
        if zipType != 'application/.zip':
            raise MyError("Failed to get requested zip file from Web Soil Survey (" + zipType + ")")
        ...
        errMsg = "Socket timeout. " + e.reason
        raise MyError(errMsg)
        return ""

    except socket.error as e:
        #PrintMsg(".\t\t" + areasym + " - Web Soil Survey connection failure", 1)
        errMsg = "Socket error. " + e.reason
        raise MyError(errMsg)
        return ""

    #except httplib.BadStatusLine as e:
        #PrintMsg(".\t\t" + areasym + " - Web Soil Survey connection failure", 1)
    #    errMsg = "HTTP bad status line. " + e.reason
    #    raise MyError(errMsg)
    #    return ""

    except KeyboardInterrupt:
        PrintMsg("Keyboard interrupt", 1)
        sys.exit()
...
try:
    ...
    if importDB == "":
        ...
        PrintMsg(" \nTabular import using database: " + str(importDB), 1)
        bImport = True
