...
def SelectAll(event):
    ...
    if totalCnt > 0:
        ...
        lblText.set("No values in listbox, totalCnt: " + str(totalCnt))
...
lbxCRS = tk.Listbox(master=frmEntries, listvariable=crsList, width=25, height=3, selectmode='single', background=ltGray, selectbackground="red", exportselection=False, fg="black", font=("Arial", 11))  #  highlightcolor="red",
...
lbxDownloads = tk.Listbox(master=frmBottom, listvariable=downloadList, width=15, height=20, selectmode='multiple', background=ltGray, selectbackground="red", exportselection=False,  fg="black", font=("Arial", 11))
...
lblCount = tk.Label(master=frmBottom, textvariable=lblText, anchor="w", width=60,padx=10, pady=5, background=gray)
lblCount.grid(row=1, column=1, columnspan=3, sticky="W")



# Label for duplicate downloads (different folder names, same areasymbol)
lblDups = tk.Label(master=frmBottom, text="Init", anchor="w", width=40, padx=10, pady=5, background=gray)
